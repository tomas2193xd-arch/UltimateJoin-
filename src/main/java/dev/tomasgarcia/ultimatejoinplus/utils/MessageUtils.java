...
import dev.tomasgarcia.ultimatejoinplus.managers.MessageProcessor;
...
public class MessageUtils {
    ...
    public static Component centerMessage(String message) {
        ...
        // Convert to plain text to calculate width, ignoring color codes
        // Note: This is an approximation. For MiniMessage, we strip tags to guess
        // length.
        String cleanMessage = MiniMessage.miniMessage().stripTags(message);
        ...
        for (char c : cleanMessage.toCharArray()) {
            if (c == '§') { // Legacy support just in case, though stripTags handles most
                previousCode = true;
        ...
        return MessageProcessor.process(sb.toString() + message);
    }

    // Enum for Character widths
